...
Collaborative text for [cf-sync](https://github.com/InfinityBowman/cf-sync): Yjs fields that live in the **same Workspace Durable Object** as your rows — one storage authority, one socket, one export/import surface. Rows stay last-write-wins; only the fields that genuinely need character-level merging pay for a CRDT.
