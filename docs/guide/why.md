...
The sync space is crowded, and most of these tools are excellent. cf-sync exists in a specific gap: *server-authoritative semantics, on Cloudflare primitives, with no separate backend to operate*. Its design borrows deliberately from prior art — the [design document](https://github.com/InfinityBowman/cf-sync-engine/blob/main/DESIGN.md) cites Replicache, Zero, LiveStore, tldraw sync, and partyserver at file:line granularity.
