...
Unless `crud: false`, the [`crudMutators`](#crudmutators) pair is spread into the registry first — your entries win on a name collision, so a hand-rolled `sync.put` overrides the built-in rather than duplicating it. `crud: false` with no mutators throws (it declares an app nothing can write to), and [collections](/reference/collections) refuse to attach to a `crud: false` app — every write must go through a named mutator. The `authContext` schema declared with `defineMutators` is lifted onto the definition here, so the server can validate authorize verdicts at connect and the client can fail-fast-validate its `auth` option.
